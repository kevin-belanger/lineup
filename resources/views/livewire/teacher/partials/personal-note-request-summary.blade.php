...
<div class="rounded-md border border-indigo-200 bg-white p-3 shadow-sm">
    @if ($supportRequest->is_priority)
        ...
        @if ($supportRequest->classroom)
            <div class="mt-2 text-sm text-gray-700">{{ $supportRequest->classroom->name }}</div>
        @endif
    @else
        ...
        @if ($subjectUrl)
            <a
                ...
                class="mt-2 block rounded-md px-2 py-1.5 text-sm font-medium leading-snug text-gray-700 transition hover:bg-indigo-50 hover:text-indigo-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
        ...
        @else
            <div class="mt-2 whitespace-normal break-words px-2 py-1.5 text-sm font-medium leading-snug text-gray-700">
                {{ $supportRequest->subject?->name ?? 'N/A' }} - {{ __('Tile') }} {{ $supportRequest->moodle_tile_number }}
            </div>
        @endif
    @endif
