<section @if ($requests->isNotEmpty()) wire:poll.3s.visible @endif class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                ...
                <div class="flex flex-col gap-8 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    @if ($supportRequest->assigned_teacher_id)
                        <button
                            ...
                            aria-label="{{ __('Annuler la demande') }}"
                        >
    ...
    @if ($confirmingAssignedCancellationId)
        <div class="fixed inset-0 z-50 flex items-center justify-center px-4 py-6">
            <button type="button" class="absolute inset-0 bg-gray-500 opacity-75" wire:click="dismissAssignedCancellation" aria-label="{{ __('Retour') }}"></button>

            <x-confirmation-panel
                :title="__('Annuler la demande')"
                :message="__('Cette demande est déjà prise en charge par un enseignant. Voulez-vous vraiment l\'annuler ?')"
            >
                <x-slot name="actions">
